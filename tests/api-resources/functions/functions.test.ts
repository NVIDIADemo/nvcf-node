...
describe('resource functions', () => {
  test('create: only required params', async () => {
    const responsePromise = client.functions.create({ inferenceUrl: 'https://example.com', name: 'x' });
  ...
  test('retrieveAll', async () => {
    const responsePromise = client.functions.retrieveAll();
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
  ...
  test('retrieveAll: request options instead of params are passed correctly', async () => {
    ...
    await expect(client.functions.retrieveAll({ path: '/_stainless_unknown_path' })).rejects.toThrow(
      NVCF.NotFoundError,
    );
  });
  ...
  test('retrieveAll: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(
      client.functions.retrieveAll({ visibility: ['authorized'] }, { path: '/_stainless_unknown_path' }),
